...
$allGrades = $pdo->prepare('SELECT SUM(grade*weight)/SUM(weight) AS average FROM grade WHERE SID = :sid AND UID = :uid'); // get all grades from a user to a $subject
...
foreach ($subjRes as $subject) {
  ...
  $subjectsArray[$subject['id']]['average'] = $allGrades->fetch(PDO::FETCH_COLUMN);
